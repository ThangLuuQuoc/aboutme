<?php
	...
	$titlePage = $messages["general_title_contents"];
	$titleAdd = $messages["general_title_contents_add"];
	$scriptAdd = "addUpdateContent.php";
	...
	if (isset ($_POST['search']) && $_POST['search'] != '') {
   	...
   	//<ORDER_BY>
	//VARS CONFIG
	$fields_sql_order_by  = array ("cont_name", "cont_date_create", "cont_status", "cont_order", "");
	$field_is_href = array (1, 1, 1, 1, 0);//para saber si es o no campo para ordenar	
	$fields_name_order_by = array ($messages["general_title"], $messages["general_date_create"], $messages["general_status"], $messages["general_order"], $messages["general_options"]);
	$item_width = array("40%", "20%", "15%", "15%", "");
	$method_order = "GET";
	$ord_default = 'ASC';
	$class_href_order = 'lbl_white';//clase del href	
	$index_order_default = 2;//por cual ordenar en primer vez
	...
	if ($order_by != "cont_order") {
		$orderByList .= ", cont_order";
	}
	//</ORDER_BY>
	
	$listRows = $content->countContents($search);
	$list = $content->contentList($search, $orderByList, $init, $amount);
	$countRows = count ($list);
	
	$arrayOrders = $content->getArrayOrders();
	$countOrders = $content->getMaxOrder(true);
