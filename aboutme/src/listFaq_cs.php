<?php
	...
	$titlePage = $messages["general_title_faqs"];
	$titleAdd = $messages["general_title_faqs_add"];
	$scriptAdd = "addUpdateFaq.php";
	...
	if (isset ($_POST['search']) && $_POST['search'] != '') {
   	...
   	//<ORDER_BY>
	//VARS CONFIG
	$fields_sql_order_by  = array ("faq_query", "faq_status", "faq_order", "");
	$field_is_href = array (1, 1, 1, 0);//para saber si es o no campo para ordenar	
	$fields_name_order_by = array ($messages["general_query"], $messages["general_status"], $messages["general_order"], $messages["general_options"]);
	$item_width = array("50%", "20%", "20%", "");
	$method_order = "GET";
	$ord_default = 'ASC';
	...
	if ($order_by != "faq_order") {
		$orderByList .= ", faq_order";
	}
	//</ORDER_BY>
	
	$listRows = $faq->countFaq($search);
	$list = $faq->faqList($search, $orderByList, $init, $amount);
	$countRows = count ($list);
	
	$countOrders = $faq->getMaxOrder(true);
