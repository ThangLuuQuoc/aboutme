<?php
	...
	$titlePage = $messages["general_title_services_type"];
	$titleAdd = $messages["general_title_services_type_add"];
	$scriptAdd = "addUpdateServiceType.php";
	...
	if (isset ($_POST['search']) && $_POST['search'] != '') {
   	...
   	//<ORDER_BY>
	//VARS CONFIG
	$fields_sql_order_by  = array ("sertype_name", "sertype_status", "sertype_order", "");
	$field_is_href = array (1, 1, 1, 0);//para saber si es o no campo para ordenar	
	$fields_name_order_by = array ($messages["general_name"], $messages["general_status"], $messages["general_order"], $messages["general_options"]);
	$item_width = array("60%", "20%", "10%", "");
	$method_order = "GET";
	$ord_default = 'ASC';
	$class_href_order = 'lbl_white';//clase del href	
	$index_order_default = 1;//por cual ordenar en primer vez
	...
	if ($order_by != "sertype_order") {
		$orderByList .= ", sertype_order";
	}
	//</ORDER_BY>
	
	$listRows = $serviceType->countServicesType($search);
	$list = $serviceType->serviceTypeList($search, $orderByList, $init, $amount);
	$countRows = count ($list);
	
	$arrayOrders = $serviceType->getArrayOrders();
	$countOrders = $serviceType->getMaxOrder(true);
