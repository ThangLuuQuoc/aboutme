<?php
	...
	$titlePage = $messages["general_title_personal_charge"];
	$titleAdd = $messages["general_title_personal_charge_add"];
	$scriptAdd = "addUpdateCharge.php";
	...
	if (isset ($_POST['search']) && $_POST['search'] != '') {
   	...
   	//<ORDER_BY>
	//VARS CONFIG
	$fields_sql_order_by  = array ("chg_name", "chg_status", "chg_order", "");
	...
	if ($order_by != "chg_order") {
		$orderByList .= ", chg_order";
	}
	//</ORDER_BY>
	
	$listRows = $charge->countCharges($search);
	$list = $charge->chargeList($search, $orderByList, $init, $amount);
	$countRows = count ($list);
	
	$arrayOrders = $charge->getArrayOrders();
	$countOrders = $charge->getMaxOrder(true);
