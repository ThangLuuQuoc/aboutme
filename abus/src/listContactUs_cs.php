<?php
	...
	$titlePage = $messages["general_title_contact_us"];
	...
	if (isset ($_POST['search']) && $_POST['search'] != '') {
   	...
   	//<ORDER_BY>
	//VARS CONFIG
	$fields_sql_order_by  = array ("contact_date_create", "contact_name", "contact_email", "contact_phone", "contact_city", "contact_status", "");
	$field_is_href = array (1, 1, 1, 1, 1, 1, 0);//para saber si es o no campo para ordenar	
	$fields_name_order_by = array ($messages["general_date_create"], $messages["general_name"], $messages["general_email"], $messages["general_phone"], 
									$messages["general_city"], $messages["general_status"], $messages["general_options"]);
	$item_width = array("10%", "20%", "20%", "15%", "15", "10%", "10%");
	$method_order = "GET";
	$ord_default = 'ASC';
	$class_href_order = 'lbl_white';//clase del href	
	$index_order_default = 5;//por cual ordenar en primer vez
	...
	if ($order_by != "contact_date_create") {
		$orderByList .= ", contact_date_create";
	}
	//</ORDER_BY>
	
	$listRows = $contactUs->countContactUs($search);
	$list = $contactUs->contactUsList($search, $orderByList, $init, $amount);
