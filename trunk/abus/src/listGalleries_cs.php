<?php
	...
	$titlePage = $messages["general_title_galleries"];
	$titleAdd = $messages["general_title_galleries_add"];
	$scriptAdd = "addUpdateGallery.php";
	...
	if (isset ($_POST['search']) && $_POST['search'] != '') {
   	...
   	//<ORDER_BY>
	//VARS CONFIG
	$fields_sql_order_by  = array ("", "gall_name", "gall_date_create", "gall_status", "gall_order", "");
	$field_is_href = array (0, 1, 1, 1, 1, 0);//para saber si es o no campo para ordenar	
	$fields_name_order_by = array ($messages["general_front"], $messages["general_name"], $messages["general_date_create"], $messages["general_status"], $messages["general_order"], $messages["general_options"]);
	...
	if ($order_by != "gall_order") {
		$orderByList .= ", gall_order";
	}
	//</ORDER_BY>
	$list = $gallery->galleriesList($search, $orderByList, $init, $amount);
	$listRows = $gallery->countGalleries($search);	
	$countRows = count ($list);
	
	$arrayOrders = $gallery->getArrayOrders();
	$countOrders = $gallery->getMaxOrder(true);
